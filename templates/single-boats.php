...
while (have_posts()) : the_post();
	...
	// OPTIMIZED: Use get_post_custom() for single DB query instead of 25+ individual calls
	// Performance: ~25 DB queries → 1 DB query (24 fewer queries)
	$all_meta = get_post_custom($post_id);
	
	$price      = $all_meta['wpbs_price'][0] ?? '';
	$length     = $all_meta['wpbs_length_overall'][0] ?? '';
	$year       = $all_meta['wpbs_model_year'][0] ?? '';
	$make       = $all_meta['wpbs_make'][0] ?? '';
	$model      = $all_meta['wpbs_model'][0] ?? '';
	$engine     = $all_meta['wpbs_engine_summary'][0] ?? '';
	$location   = $all_meta['wpbs_location'][0] ?? '';
	$status     = $all_meta['wpbs_sales_status'][0] ?? '';
	$hull_type  = $all_meta['wpbs_hull_material'][0] ?? '';
	$fuel_type  = $all_meta['wpbs_fuel_type'][0] ?? '';
	$condition  = $all_meta['wpbs_condition'][0] ?? '';
	$total_power = $all_meta['wpbs_total_engine_power'][0] ?? '';
	$engine_hrs = $all_meta['wpbs_total_engine_hours'][0] ?? '';
	$num_engines = $all_meta['wpbs_number_of_engines'][0] ?? '';
	$dealer     = $all_meta['wpbs_dealer_name'][0] ?? '';
	$beam       = $all_meta['wpbs_beam'][0] ?? '';
	$dry_weight = $all_meta['wpbs_dry_weight'][0] ?? '';
	$cabins     = $all_meta['wpbs_cabins_count'][0] ?? '';
	$heads      = $all_meta['wpbs_heads_count'][0] ?? '';
	$engine_type = $all_meta['wpbs_engine_type'][0] ?? '';
	$engine_make = $all_meta['wpbs_engine_make'][0] ?? '';
	$engine_model = $all_meta['wpbs_engine_model'][0] ?? '';
	$propeller  = $all_meta['wpbs_propeller_type'][0] ?? '';
	$drive_type = $all_meta['wpbs_drive_type'][0] ?? '';
	$boat_category = $all_meta['wpbs_boat_category'][0] ?? '';
	$boat_class = $all_meta['wpbs_boat_class_codes'][0] ?? '';
	$hull_id    = $all_meta['wpbs_hull_id'][0] ?? '';
	$cruising_speed = $all_meta['wpbs_cruising_speed'][0] ?? '';
	$max_speed  = $all_meta['wpbs_max_speed'][0] ?? '';
	$fuel_capacity = $all_meta['wpbs_fuel_tank_capacity'][0] ?? '';
	$water_capacity = $all_meta['wpbs_water_tank_capacity'][0] ?? '';
	$office_phone = $all_meta['wpbs_office_phone'][0] ?? '';
	$office_email = $all_meta['wpbs_office_email'][0] ?? '';
	$deadrise = $all_meta['wpbs_deadrise'][0] ?? '';
